...
export const DOMAINS: Domain[] = [
  {
    ...
    expertPitch: "Software engineers contribute across open dataset templates.",
  },
  {
    id: "legal",
    name: "Legal",
...
/** The stages this deployment's community pipeline actually runs, in the order
 *  `apps/api/src/services/validation.ts` runs them.
 *
 *  Two former members are deliberately gone:
 *  - `"contamination"` — external-corpus plagiarism screening was removed from
 *    the pipeline entirely by owner decision. No code path writes a
 *    `contamination` stage row and there is deliberately no stub standing in
 *    for one, so no public surface may claim it runs.
 *  - `"schema"` — it survives only in an admin dataset-type allowlist. No code
 *    path executes a schema stage: `DatasetType.fields` is read to build the
 *    execution harness and the LLM prompt, never as a submit-time gate.
 *
 *  `"ai_attribution"` is unconditional and not configurable per type — the API
 *  excludes it from its allowlist precisely because a sponsor or admin must not
 *  be able to opt a type in or out of it. `"human_audit"` is likewise always
 *  the final acceptance decision (`pendingHumanReview` is set unconditionally).
 *  `"llm"` is the one conditional stage: it needs the `validation.llm.enabled`
 *  platform switch, and is dropped from the pipeline outright when that is off. */
...
export interface DatasetType {
  ...
  usageCount: number; // bounties launched with this type
}
...
export const DATASET_TYPE_CATALOG: DatasetType[] = [
  /* ---------- coding · the original five ---------- */
  {
    ...
    verification: {
      pipeline: ["dedupe", "ai_attribution", "execution", "llm", "human_audit"],
  ...
  },
  {
    ...
    verification: {
      pipeline: ["dedupe", "ai_attribution", "llm", "human_audit"],
  ...
  },
  {
    ...
    verification: {
      pipeline: ["dedupe", "ai_attribution", "execution", "human_audit"],
  ...
  },
  {
    ...
    verification: {
      pipeline: ["dedupe", "ai_attribution", "llm", "human_audit"],
      dedupeFields: ["document_excerpt", "identified_anomaly_or_insight"],
      auditOptions: [100],
    },
    trustTier: "expert_audited",
    difficultyLevels: ["intermediate", "expert"],
