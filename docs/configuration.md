...
| `REDIS_URL` ✱ | *(empty)* | Connection string used when `CACHE_DRIVER=redis`. On the dev deployment this is `redis://redis:6379` — an internal Docker hostname, not published, so no credentials. The driver fails open behind a circuit breaker: if Redis is unreachable the app degrades to the database path instead of waiting out timeouts. **Every key this application writes is namespaced `databounty:`**, so the instance may safely be shared with another application — as it is in production, where the Redis also serves a co-tenant on the same host. What is cached: public catalogue reads (invalidated explicitly on write) and verified API keys (60s, dropped immediately on revoke, rotation and account suspension — a cached credential never outlives the decision that revoked it). |
